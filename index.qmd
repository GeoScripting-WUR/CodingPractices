...
format:
  html:
    title: "AI and best coding practices"
    theme: simplex
...
The most popular IDEs for R are developed by Posit, a company that is very active in contributing to R (Hadley Wickham and others). 
The first IDE Posit developed is called *RStudio*, and it continues to be very popular.
Its successor is *Positron*, which is specifically designed with (bigger) data analysis and data science in mind.
It is designed not only for R, but also for Python, Bash and other languages. 
Positron is based upon Visual Studio Code, a popular multi-language IDE developed by Microsoft.
Although Posit will keep maintaining RStudio, new features will be [primarily implemented](https://positron.posit.co/faqs.html) in Positron.
Both RStudio and Positron are cross-platform and open-source. 


Even though the Posit IDEs are very popular, they are not the only IDEs for R.
...
Furthermore, there are more cross-language GUIs/IDEs like Positron, such as *Jupyter*, whose name is a portmanteau of **Ju**lia, **Pyt**hon and **R**.
*Jupyter* can run different language interpreters, what it calls kernels, for each script that is opened.
Therefore, scripts in multiple languages can be edited at the same time in the same interface.
...
We already used in in the previous tutorials to interact with Git and Bash, and it will be our go-to IDE throughout the course, in both R and Python.

See [this introduction](https://posit.co/blog/a-quick-tour-of-positron) by Posit to get familiar with the layout and functionality. 
...
If you are using RStudio, the first time you open it, it's helpful to change some global settings.
...
* As we will be working with multiple languages throughout this course, we will keep things organized by placing scripts in a directory per language.
  ...
  * __Python __
    * For Python we will follow coding conventions, for more detailed information see the [Hitchhiker's guide to Python](https://docs.python-guide.org/writing/structure/#structure-of-code-is-key).
    * In python functions are stored in a directory with a name relevant to the project, in a file with a relevant filename. For example: 
      * A small (< 10 functions) flood mapping exercise, you would put functions in a file called `functions.py` in a directory called `floodmapping`. 
      * For a bigger traffic dashboard project with separate download, analysis and publishing steps, functions and classes could go in a directory called `trafficdashboard` in files called `download.py`, `analysis.py` and `publishing.py`. 
    * Use scoping to keep things organized. To import functions into the main script write `from floodmapping import functions as funcs`. Call a function using the scope: `funcs.myfunction` The scoping allows self written functions to be distinguishable from other functions. 
